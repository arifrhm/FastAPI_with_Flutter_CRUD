...
async def fetch_transaksi(
    ...
    out_response = await transaksi_crud.fetch_users(db_session=db_session)
...
async def update_transaksi(
    ...
    out_response = await transaksi_crud.update_user(id=id, request=request, db_session=db_session )
    handle_error_code(out_response,response)
    return out_response
